...
import numpy as np
import pandas as pd
from model import MLPSurNameClassifier, CNNSurNameClassifier
...
class SurnameVectorizer(object):
    ...
    def vectorize(self, surname):
        one_hot = np.zeros(len(self._surname_vocab), dtype=np.float32)
        for token in surname:
            one_hot[self._surname_vocab.lookup_token(token)] = 1
        return one_hot
